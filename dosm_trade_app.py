...
# selection menu
graph = st.sidebar.selectbox('Type of graph:', ['Explore Trades Performance!', 'Explore Trades Strength!','Explore Trades Dynamic!', 'TIME-SERIES', 'BAR CHART', 'PIE CHART'])
# graph = st.sidebar.selectbox('Type of graph:', ['Explore Trades Performance!','Explore Trades Dynamic!','Explore','TIME-SERIES', 'BAR CHART', 'PIE CHART', 'AREA PLOT'])


if (graph == 'PIE CHART') or (graph == 'Explore Trades Dynamic!') or (graph == 'Explore Trades Strength!') or (graph == 'Explore Trades Performance!'):
    ...
    if st.button('Show Growth Rate'):
        if len(show) == 0:
            ...
            for metric in show:
                ...
                list.append(lol)
            year[list].dropna().plot(ax=ax1, figsize=(12, 8),
                                     title="Growth Rate of Malaysia's Trade Perfomance from 2014 to 2019")
            st.pyplot(fig1)


def bi_graph_line(column, i):
    fig, ax = plt.subplots(1, 1)
    df[df[column] == i].groupby('YEAR')[show].sum().plot(ax=ax, figsize=(12, 8),
                                                         title="Trade Performance of Malaysia in {}: {} from 2013 to 2019".format(
                                                             column, i))
    st.pyplot(fig)
    if st.button('Show Datasets'):
    ...
    if st.button('Show Growth Rate'):
        if (len(show) == 0):
            ...
            for metric in show:
                ...
                list.append(lol)
            year[list].dropna().plot(ax=ax1, figsize=(12, 8),
                                     title="Growth Rate of Malaysia's Trade Perfomance in {}: {} from 2014 to 2019".format(
                                         column, i))
            st.pyplot(fig1)
...
def multiple_graph_line(column_1, i, column_2, j):
    fig, ax = plt.subplots(1, 1)
    df[(df[column_1] == i) & (df[column_2] == j)].groupby('YEAR')[show].sum().plot(ax=ax, figsize=(12, 8),
                                                                                   title="Trade Performance of Malaysia in {}: {} and {}: {} from 2013 to 2019".format(
                                                                                       column_1, i, column_2, j))
    st.pyplot(fig)
    if st.button('Show Datasets'):
        ...
        st.markdown(get_table_download_link(df[(df[column_1] == i) & (df[column_2] == j)].groupby('YEAR')[show].sum()),
                    unsafe_allow_html=True)
    if 'DEFICIT/SURPLUS (MILLION RM)' in show:
        show.remove('DEFICIT/SURPLUS (MILLION RM)')
    if st.button('Show Growth Rate'):
        if (len(show) == 0):
            ...
            for metric in show:
                ...
                list.append(lol)
            year[list].dropna().plot(ax=ax1, figsize=(12, 8),
                                     title="Growth Rate of Malaysia's Trade Perfomance in {}: {} and {}: {} from 2014 to 2019".format(
                                         column_1, i, column_2, j))
            st.pyplot(fig1)
...
def single_graph_bar(column, sort, k):
    fig, ax = plt.subplots(1, 1)
    df.groupby(column)[show].sum().sort_values(by=sort, ascending=False).iloc[:k].plot(kind='bar', ax=ax,
                                                                                       figsize=(12, 8),
    ...
    if st.button('Show Datasets'):
        ...
        st.markdown(
            get_table_download_link(df.groupby(column)[show].sum().sort_values(by=sort, ascending=False).iloc[:k]),
            unsafe_allow_html=True)
...
def bi_graph_bar(column_1, column_2, i, sort, k):
    ...
    fig, ax = plt.subplots(1, 1)
    df[df[column_2] == i].groupby(column_1)[show].sum().sort_values(by=sort, ascending=False).iloc[:k].plot(kind='bar',
                                                                                                            ax=ax,
                                                                                                            figsize=(
                                                                                                            12, 8),
                                                                                                            title=title)
    ...
    if st.button('Show Datasets'):
        ...
        st.markdown(get_table_download_link(
            df[df[column_2] == i].groupby(column_1)[show].sum().sort_values(by=sort, ascending=False).iloc[:k]),
                    unsafe_allow_html=True)


def multiple_graph_bar(column_1, column_2, i, column_3, j, sort, k):
    fig, ax = plt.subplots(1, 1)
    df[(df[column_2] == i) & (df[column_3] == j)].groupby(column_1)[show].sum().sort_values(by=sort,
                                                                                            ascending=False).iloc[
    :k].plot(kind='bar', ax=ax, figsize=(12, 8),
             title="Trade Performance of Malaysia based on {} in {}: {} and {}: {}".format(column_1, column_2, i,
                                                                                           column_3, j))
    st.pyplot(fig)
    if st.button('Show Datasets'):
        st.text(
            "Trade Performance of Malaysia based on {} in {}: {} and {}: {}".format(column_1, column_2, i, column_3, j))
        st.dataframe(df[(df[column_2] == i) & (df[column_3] == j)].groupby(column_1)[show].sum().sort_values(by=sort,
                                                                                                             ascending=False).iloc[
                     :k])
        st.markdown(get_table_download_link(
            df[(df[column_2] == i) & (df[column_3] == j)].groupby(column_1)[show].sum().sort_values(by=sort,
...
def single_pie(column, k):
    ...
    fig, ax = plt.subplots()
    df1.groupby(column)[[show]].sum().sort_values(by=show, ascending=False).plot.pie(y=show, autopct='%1.1f%%', ax=ax,
    ...
    if st.button('Show Datasets'):
        ...
        st.markdown(get_table_download_link(df1.groupby(column)[[show]].sum().sort_values(by=show, ascending=False)),
                    unsafe_allow_html=True)
...
def bi_pie(column_1, k, column_2, i):
    ...
    fig, ax = plt.subplots(1, 1)
    df1.groupby(column_1)[[show]].sum().sort_values(by=show, ascending=False).plot.pie(y=show, autopct='%1.1f%%', ax=ax,
                                                                                       figsize=(12, 8), legend=False,
                                                                                       title=title)
    st.pyplot(fig)
    if st.button('Show Datasets'):
        ...
        st.markdown(get_table_download_link(df1.groupby(column_1)[[show]].sum().sort_values(by=show, ascending=False)),
                    unsafe_allow_html=True)


def multiple_pie(column_1, k, column_2, i, column_3, j):
    pie = df[(df[column_2] == i) & (df[column_3] == j)].groupby(column_1)[[show]].sum().sort_values(by=show,
                                                                                                    ascending=False)
    q = pie[show].quantile((len(pie) - k) / (len(pie)))
    df1 = pie.reset_index()
    df1.loc[df1[show] < q, column_1] = 'OTHERS'
    fig, ax = plt.subplots(1, 1)
    df1.groupby(column_1)[[show]].sum().sort_values(by=show, ascending=False).plot.pie(y=show, autopct='%1.1f%%',
                                                                                       legend=False, ax=ax,
                                                                                       figsize=(12, 8),
                                                                                       title="Trade Performance of Malaysia based on {} in {}: {} and {}: {}".format(
                                                                                           column_1, column_2, i,
                                                                                           column_3, j))
    st.pyplot(fig)
    if st.button('Show Datasets'):
...
def multiple_graph_tree(feature, column_1, i, column_2, j, ):
    try:
        title = "Trade Performance of Malaysia based on {} in {}: {} and {}: {}".format(feature, column_1, i, column_2,
                                                                                        j)
        if feature == 'COUNTRY':
...
def tree_map():
    ...
    st.subheader("Trades (Import or Export) perfomance could be analyzed using TREE MAP.")
    if tree == 'COMMODITY':
        tree_map_good()
    elif tree == 'DIRECTION':
        tree_map_should()
...
def bubble_graph():
    st.subheader("Trades (Import or Export) dynamic could be analyzed using BUBBLE PLOT.")
    st.subheader("Bubble Plot of Malaysia's Trade's from 2013 to 2019")
    ...
    fig = px.scatter(df_import_dynamic.query("YEAR=={}".format(year)), x="RCA Value", y="Percent Change", size=show,
                     color="SITC 1 DIGIT", hover_name="1D DESC", log_x=False, size_max=60)
...
def area_graph():
    st.subheader("Trades (Import or Export) strength could be analyzed using AREA GRAPH")
...
# 'Explore Trades Performance','Explore Trades Dynamic','Explore Trades Strength'
def plot_graph():
    if graph == 'PIE CHART':
        pie_graph()
    elif graph == 'TIME-SERIES':
        line_graph()
    elif graph == 'Explore Trades Performance!':
        tree_map()
    elif graph == 'Explore Trades Dynamic!':
        bubble_graph()
    elif graph == 'Explore Trades Strength!':
        area_graph()
    else:
        bar_graph()
